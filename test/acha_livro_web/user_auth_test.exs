defmodule AchaLivroWeb.UserAuthTest do
  use AchaLivroWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias AchaLivro.Accounts
  alias AchaLivro.Accounts.Scope
  alias AchaLivroWeb.UserAuth

  import AchaLivro.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AchaLivroWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      AchaLivroWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        AchaLivroWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
